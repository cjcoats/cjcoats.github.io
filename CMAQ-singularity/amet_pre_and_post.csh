...
set AMETOUT   = /work/SCRATCH/output
...
setenv SINGULARITYENV_RUN_COMBINE       F     #> T/F; Run combine on CCTM output?
setenv SINGULARITYENV_RUN_HR2DAY        F     #> T/F; Run hr2day program (needed for analyzing TOAR network data)
setenv SINGULARITYENV_WRITE_SITEX       T     #> T/F; Write scripts for running site compare for each selected network?
setenv SINGULARITYENV_RUN_SITEX         T     #> T/F; Run site compare scripts for each selected network?

#> AMET options
setenv SINGULARITYENV_CREATE_PROJECT    T     #> T/F; Create AMET project?
setenv SINGULARITYENV_LOAD_SITEX        T     #> T/F; Load site compare output for each selected network into AMET?
setenv SINGULARITYENV_UPDATE_PROJECT    F     #> T/F; Update the project info for an existing project (all data are retained)
setenv SINGULARITYENV_REMAKE_PROJECT    F     #> T/F; Remake an existing AMET project. Note that all existing data will be deleted
setenv SINGULARITYENV_DELETE_PROJECT    F     #> T/F; Delete an existing AMET project. This will delete all data in the existing
                                #>      AMET table and remove the table from the database

#> Plotting options
setenv SINGULARITYENV_AMET_DB           T     #> T/F; Set to T if the model/obs pairs are loaded in the AMET database (i.e. by setting LOAD_SITEX = T)
                                #>      When set to F, plotting scripts will read the sitecmp .csv files directly
setenv SINGULARITYENV_spatial_plots     T     #> T/F; Create maps of biase and error from site compare output?
setenv SINGULARITYENV_stacked_barplots  T     #> T/F; Create stacked bar plots of PM2.5 species from site compare output?
setenv SINGULARITYENV_time_plots        T     #> T/F; Create time series plots from site compare output?
setenv SINGULARITYENV_scatter_plots     T     #> T/F; Create scatter plots from site compare output?
setenv SINGULARITYENV_misc_plots        T     #> T/F; Create bugle plots and soccer goal plots from site compare output?
